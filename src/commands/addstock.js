...
export const data = new SlashCommandBuilder()
  .setName('addstock')
  .setDescription('Admin only add stock quantity to an item')
...
export async function execute(interaction) {
  ...
  await query(
    `UPDATE stock_items SET stock_quantity = stock_quantity + $2, updated_at = NOW() WHERE item_id = $1`,
    [itemId, quantity]
  );
  await interaction.reply({ content: 'Stock added.', ephemeral: true });
}
